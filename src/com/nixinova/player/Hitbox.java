...
public class Hitbox {
	...
	private static Map<Corner, Vector3.SmallInt> playerCornerOffsets = new HashMap<>() {
		private static final long serialVersionUID = 1L;
		{
			put(Corner.FOOT_xz, new Vector3.SmallInt(-1, 0, -1));
			put(Corner.FOOT_Xz, new Vector3.SmallInt(+1, 0, -1));
			put(Corner.FOOT_xZ, new Vector3.SmallInt(-1, 0, +1));
			put(Corner.FOOT_XZ, new Vector3.SmallInt(+1, 0, +1));

			put(Corner.HEAD_xz, new Vector3.SmallInt(-1, 1, -1));
			put(Corner.HEAD_Xz, new Vector3.SmallInt(+1, 1, -1));
			put(Corner.HEAD_xZ, new Vector3.SmallInt(-1, 1, +1));
			put(Corner.HEAD_XZ, new Vector3.SmallInt(+1, 1, +1));
		}
	...
	public static CornersList getCollisionPoints(World world, Coord3 position) {
		...
		for (var hitboxEntry : playerCornerOffsets.entrySet()) {
			...
			Vector3.SmallInt corner = hitboxEntry.getValue();

			// get block at each corner of the player's hitbox
			short posX = Coord1.fromSubBlock(blockPos.x + Player.PLAYER_WIDTH / 2 * corner.x).toBlock();
			short posY = Coord1.fromSubBlock(blockPos.y + Player.PLAYER_HEIGHT * corner.y).toBlock();
			short posZ = Coord1.fromSubBlock(blockPos.z + Player.PLAYER_WIDTH / 2 * corner.z).toBlock();
